...
class Product(osv.osv):
    ...
    def _partner_name2(self, cursor, user, ids, name, arg, context=None):
        res = {}
        for product in self.browse(cursor, user, ids, context=context):
            res[product.id] = '\n'.join([x.product_name \
                    for x in product.seller_ids if x.product_name]) or ''
        return res

    _columns = {
        'partner_ref2': fields.function(_partner_ref2, method=True,
            type='char', string='Customer ref'),
        'partner_name2': fields.function(_partner_name2, method=True,
            type='char', string='Customer name'),
    }
