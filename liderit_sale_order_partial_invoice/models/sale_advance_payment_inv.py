...
class SaleAdvancePaymentInv(models.TransientModel):
    ...
    sale_order_line_id = fields.One2many(
        comodel_name='sale.advance.payment.inv.line', string="Order Lines",
    ...
    def make_partial_invoice(self):
        ...
        for line in lines:
            if line.partial_invoice:
                if line.partial_invoice < 0:
                    # con el simbolo _() marcamos el texto para exportar a traduccion
                    message = _("You can't invoice lees quantity than zero.")
                    error_message += message
                if line.partial_invoice > line.quantity:
                    message = _("You can't invoice more quantity than sold in order")
                    error_message += message
                if line.sale_order_line_id.state == 'done':
                    message = _("This is yet invoiced")
                    error_message += message
                if line.sale_order_line_id.state == 'cancel':
                    message = _("This line is cancel")
                    error_message += message
            ...
            if len(error_message) == 0:
                ...
                to_invoice = line.partial_invoice
                remain = line.quantity - to_invoice
                if remain != 0:
