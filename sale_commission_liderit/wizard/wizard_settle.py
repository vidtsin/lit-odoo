    ...
    def action_settle(self):
        ...
        for agent in self.agents:
            ...
            agent_lines = agent_line_obj.search(
                ...
                #OJO: Modificacion core para poner un intervalo de tiempo invoice_date
                 ('invoice_date', '<=', date_to),
                 ('agent', '=', agent.id),
                 ('settled', '=', False)], order='invoice_date')
            logging.error("####### He encontrado facturas para liquidar: %s"%agent_lines)
            if agent_lines:
                pos = 0
                ...
                while pos < len(agent_lines):
                    ...
                    if agent_lines[pos].invoice_date > sett_to:
                        ...
                        logging.error("####### nocumple if invoice_date < 1-1-1900 #######")
                        pos += 1
                        continue
                    pos += 1
