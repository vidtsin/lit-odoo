...
class SaleOrder(models.Model):
    ...
    payment_ids = fields.Many2many(
        ...
        domain=[('account_id.type', 'in', ('receivable','other'))],
        copy=False,
    )
    ...
    @api.multi
    def add_return(self, journal_id, amount, date=None, description=None):
        """ Generate payment move lines of a certain amount linked
        with the sale order.
        """
        self.ensure_one()
        ...
        if date is None:
            date = self.date_order
        journal = journal_model.browse(journal_id)
        self._add_return(journal, amount, date, description)
        return True
    ...
    def _add_payment(self, journal, amount, date, description=None):
        ...
        move_name = self._get_payment_move_name(journal, period)
        if (description):
            move_vals = self._prepare_payment_move_wref(move_name, description, journal,
                                               period, date)
        else:
    ...
    @api.multi
    def _add_return(self, journal, amount, date, description=None):
        """ Generate move lines entries to pay the sale order. """
        move_model = self.env['account.move']
        period_model = self.env['account.period']
        period = period_model.find(dt=date)
        ...
        if (description):
            ...
            move_vals = self._prepare_payment_move(move_name, journal,
                                               period, date)
        move_lines = self._prepare_return_move_lines(move_name, journal,
                                                      period, amount, date)
    ...
    @api.multi
    def _prepare_payment_move_wref(self, move_name, ref, journal, period, date):
        return {'name': move_name,
                'journal_id': journal.id,
                'date': date,
                'ref': ref,
    ...
    def _prepare_payment_move_lines(self, move_name, journal, period,
                                    amount, date):
        #10-3-16 tenemos que crear una conciliacion para dar por conciliado el asiento de pago
        acc_conciliate = self.env['account.move.reconcile']
        values = {
            'name': move_name,
            'type': "auto",
        }
        acc_conciliate_id = acc_conciliate.create(values)
        ...
        #10-3-16 incluimos si esta registrada la cuenta para cobros de tercap
        if (partner.property_account_cobros_tercap):
            cta_cliente = partner.property_account_cobros_tercap.id
        else:
            cta_cliente = partner.property_account_receivable.id
        credit_line = {
            'name': move_name,
            'debit': 0.0,
            'credit': amount,
            'account_id': cta_cliente,
            'journal_id': journal.id,
            'period_id': period.id,
            'partner_id': partner.id,
            'date': date,
            ...
            'currency_id': currency.id,
            'sale_ids': [(4, self.id)],
            #agregamos el id del movimiento de conciliacion
            'reconcile_id': acc_conciliate_id.id,
        }
        return debit_line, credit_line

    @api.multi
    def _prepare_return_move_lines(self, move_name, journal, period,
                                    amount, date):
        #10-3-16 tenemos que crear una conciliacion para dar por conciliado el asiento de pago
        acc_conciliate = self.env['account.move.reconcile']
        values = {
        ...
        return debit_line, credit_line
        _logger.error('Valores de credit line en pay_method %s'%credit_line)
...
#agregado 10-3-16 para controlar cuenta alternativa para cobros de Tercap
class ResPartner(models.Model):
    _inherit = 'res.partner'

    property_account_cobros_tercap = fields.Many2one(
        comodel_name='account.account',
        string='Cuenta Cobro Tercap',
        ondelete='cascade',
    )
