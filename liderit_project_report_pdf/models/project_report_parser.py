...
class LideritProjectReportParser(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context=None):
        super(LideritProjectReportParser, self).__init__(cr, uid, name, context=context)
    ...
    def get_task_model(self, name):
        wizard_record = request.env['liderit.wizard.project.report']
        if len(wizard_record) > 0:
            wizard_record = request.env['liderit.wizard.project.report'].search([])[-1]
        else:
            return False
...
class PrintReportProject(osv.AbstractModel):
    ...
    _name = 'report.liderit_project_report_pdf.liderit_project_report_transition'
    _inherit = 'report.abstract_report'
    _template = 'liderit_project_report_pdf.liderit_project_report_transition'
    _wrapped_report_class = LideritProjectReportParser
