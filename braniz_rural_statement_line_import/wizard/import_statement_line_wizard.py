...
class AccountBankStatementLineImport(models.TransientModel):
    ...
    absl_data = fields.Binary(string='Archivo', required=True)
    absl_fname = fields.Char(string='Nombre de archivo')
    lines = fields.Binary(
    ...
    csv_separator = fields.Selection(
        [(',', ' . (comma)'), (';', ', (semicolon)')],
        string='Separador CSV', required=True)
    ...
    def _remove_leading_lines(self, lines):
        ...
        if not header:
            raise Warning(
                _("No hay linea de cabecera en el fichero !"))
        output = input.read()
    ...
    def _input_fields(self):
        ...
        res = {
            ...
            'fecha': {'method': self._handle_date, 'required': True},
            # 'value date': {'method': self._handle_val_date},
            'fecha valor': {'method': self._handle_val_date},
            # 'val_date': {'method': self._handle_val_date},
            #'amount': {'method': self._handle_amount, 'required': True},
            'concepto': {'method': self._handle_name},
            'importe': {'method': self._handle_amount, 'required': True},
            'total': {'method': self._handle_amount, 'required': True},
            
            # 'total divisa': {'method': self._handle_currency},
            #'partner': {'method': self._handle_partner},
            # 'divisa': {'method': self._handle_currency_id},
            #'Empresa': {'method': self._handle_partner},
    ...
    def _handle_currency(self, field, line, statement, absl_vals):
        if 'amount_currency' not in absl_vals:
            #_logger.error('##### AIKO ###### Valor de campo en handle currency: %s' % field)
            #_logger.error('##### AIKO ###### Valor de dato en handle currency: %s' % line[field])
            amount_currency = str2float(line[field], self.decimal_separator)
            absl_vals['amount_currency'] = amount_currency
            #_logger.error('##### AIKO ###### Salgo de handle currency con: %s' % amount_currency)
    ...
    def _handle_partner(self, field, line, statement, absl_vals):
        if not absl_vals.get('partner_id'):
            ...
            dom_ref = dom + [('ref', 'ilike', input)]
            partners = part_mod.search(dom_ref)
            if not partners:
                dom_name = dom + [('name', 'ilike', input)]
    ...
    def _handle_currency_id(self, field, line, statement, absl_vals):
        if not absl_vals.get('currency_id'):
            input = line[field]
            curr_mod = self.env['res.currency']
            dom_ref = [('name', '=', input)]
            currencys = curr_mod.search(dom_ref)
            if not currencys:
                msg = _("Currency '%s' not found !") % input
                self._log_line_error(line, msg)
                return
            elif len(currencys) > 1:
                msg = _("Multiple currencys with Reference "
                        "or Name '%s' found !") % input
                self._log_line_error(line, msg)
                return
            else:
                currency = currencys[0]
                absl_vals['currency_id'] = currency.id
