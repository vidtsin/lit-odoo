...
class AccountInvoice(models.Model):
    ...
    def _can_be_reconciled(self):
        ...
        if not (payments and self.move_id):
            _logger.error('##### AIKO ###### Reconcile invoice. No puede conciliar por payments')
            return False
        # Check currency
        ...
        for payment in payments:
            ...
            if currency != self.currency_id:
                _logger.error('##### AIKO ###### Reconcile invoice. No puede conciliar por currency')
                return False
        return True

    @api.model
    def _get_sum_invoice_move_line(self, move_lines, invoice_type):
        _logger.error('##### AIKO ###### Reconcile invoice en sum_invoice_move con tipo%s'%invoice_type)
    ...
    def _get_sum_payment_move_line(self, move_lines, invoice_type):
        _logger.error('##### AIKO ###### Reconcile invoice en sum_paym_move con tipo%s'%invoice_type)
        if invoice_type in ['in_refund', 'out_invoice']:
            line_type = 'credit'
        else:
            line_type = 'debit'
    ...
    def _get_sum_move_line(self, move_lines, line_type):
        _logger.error('##### AIKO ###### Reconcile invoice en sum_move_line con tipo.%s'%line_type)
        res = {
            'max_date': False,
        ...
        _logger.error('##### AIKO ###### Reconcile invoice en sum_move_line sale con valor.%s'%res)
        return res
    ...
    @api.multi
    def _reconcile_invoice(self, sale_pays):
        self.ensure_one()
        company_currency = self.company_id.currency_id
        currency = self.currency_id
        use_currency = currency != company_currency
        _logger.error('##### AIKO ###### Reconcile invoice en import_control. Entro en funcion.')
        #if self._can_be_reconciled():
        #_logger.error('##### AIKO ###### Reconcile invoice. Puede conciliar')
            ...
            #                                              self.type)
        res_payment = self._get_sum_payment_move_line(sale_pays,
                                                          self.type)
        res_invoice = self._get_sum_invoice_move_line(sale_pays,
                                                          self.type)
        lines = res_invoice['lines'] + res_payment['lines']
        #if not self._lines_can_be_reconciled(lines):
        #    _logger.error('##### AIKO ###### Reconcile invoice. Lineas no concilian')
        #    return
        if not use_currency:
            balance = abs(res_invoice['total_amount'] -
                              res_payment['total_amount'])
            _logger.error('##### AIKO ###### Reconcile invoice. Balance es :%s'%balance)
            #if lines and currency.is_zero(balance):
            if lines and balance==0:
                _logger.error('##### AIKO ###### Reconcile invoice. Conciliando:%s'%lines)
                lines.reconcile()
        else:
            balance = abs(res_invoice['total_amount_currency'] -
                              res_payment['total_amount_currency'])
            _logger.error('##### AIKO ###### Reconcile invoice else. Balance es :%s'%balance)
            if lines and currency.is_zero(balance):
