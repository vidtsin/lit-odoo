...
class SaleOrderLine(models.Model):
    ...
    def product_id_change(self, cr, uid, ids,
        ...
        # _logger.error('######## AIKO valor de line en el product_change  %s'%line)
        line_qty = 1
        if line.product_uom_qty:
            line_qty = line.product_uom_qty
        if line and qty != line_qty and line.reservation_ids:
...
########################################################
    ...
    def create(self, values):        
        ...
        validity = date.today() 
        validity = validity + timedelta(days=30)
        product_uos = line.product_uos.id if line.product_uos else False
        location_id = self.env['stock.reservation']._default_location_id()
        location_dest_id = self.env['stock.reservation']._default_location_dest_id()


        values_reserva['product_id'] = line.product_id.id
        values_reserva['product_uom'] = line.product_uom.id
        values_reserva['product_uom_qty'] = line.product_uom_qty
        values_reserva['date_validity'] = validity
        values_reserva['name'] = u"%s (%s)" % (line.order_id.name, line.name)
        values_reserva['location_id'] =  location_id
        values_reserva['location_dest_id'] = location_dest_id
        values_reserva['note'] = 'Reserva automatica en la generacion de linea de pedido'
        values_reserva['product_uos_qty'] = line.product_uos_qty
        values_reserva['product_uos'] = product_uos
        values_reserva['price_unit'] = line.price_unit
        values_reserva['sale_line_id'] = line.id
        #values_reserva['restrict_partner_id'] = self.owner_id.id
          
        reserv = self.env['stock.reservation'].create(values_reserva)
        reserv.reserve()     
