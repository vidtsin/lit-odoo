            ...
            if line:
                ...
                if ter_tipodoc == 8 or (ter_tipodoc == 0 and es_nuevo == 0) or (ter_tipodoc == 1 and es_nuevo == 0):
                    ...
                    tercap_impdoc_obj = tercap_impdoc.search(cr,uid,search_condition)
                        ...
                        #raise osv.except_osv(('Error!'),('El número de pedido de Tercap está repetido. No se puede realizar la importación de datos. Revise la configuración de la aplicación de Tercap y comunique la situación a soporte de Lider para rectificar el fichero de datos a importar')) 
                    ...
                    tiene_lineas = carga_datos_lineas(self, cr, uid, ter_numdoc, ter_tipodoc, order_id, ruta, ficlineas,ter_dto,ter_dtopp,name_pedido, ter_direntrega, context=None)
...
def carga_datos_lineas(self, cr, uid, numdoc, tipodoc, order_id, ruta, file, dto_cab, dto_pp, name_pedido, ter_direntrega, context=None):
    #posicion_obj = self.pool.get('account.fiscal.position')
    ...
    for nombre in glob.glob(file):
        ...
        for lines in cln:
            if lines:
                #line [0] = NumDocumento
                #line [1] = FechaDocumento; se utiliza la de cabecera
                #line [2] = TipoDocumento
                #line [3] = NumLinea
                #line [4] = TipoLinea: 1=venta, 2=regalo, 4=retirada mercancia
                #line [5] = CodProducto; este no se usa, es uno interno de Tercap
                #line [6] = Descripcion
                #line [7] = Unidad Venta (U, Kg, C)
                #line [8] = Lote; no se trata
                #line [9] = Cantidad
                #line [10] = Cajas
                #line [11] = Peso Unidad; no se trata
                #line [12] = Peso Total ... valor para unidad si Ud. Venta es Kg
                #line [13] = Precio Bruto por unidad de producto
                #line [14] = Descuento1
                #line [15] = Descuento2
                #line [16] = Precio neto Unitario tras descuentos
                #line [17] = Indica si el precio esta fijado manualmente
                #line [18] = Tipo de IVA aplicable a la linea
                #line [19] = Tipo de Recargo aplicable a la linea
                #line [20] = Importe del IVA de la linea
                #line [21] = Importe del Recargo de la linea
                #line [22] = CodProducto Alternativo: es el id en Odoo del producto
                #line [23] = Tipo Dto1 : vacío o %
                #line [24] = Tipo Dto2: vacío, % o P (euros)
                #line [25] = Precio coste; no se trata
                ...
                if ter_tipodoclin == tipodoc and ter_numdoc == numdoc:
                    ...
                    if (ter_codproducto!=0):
                        ...
                        #29-4-16 nos falta registrar el precio de coste del producto para controlar margenes
                        #no usamos la posible informacion de precio de coste que nos da Tercap
                ...
                res = 0
        fln.close()
    #21-3-16 si devolvemos res = 0 no hay lineas, si hay lineas de devolucion devolvemos 1, si no devolvemos 99
    return res
...
def retornar_pedido (self, cr, uid, ter_codproducto, ter_descripcion, qty_devol, valor_uom_id, name_pedido, ter_direntrega, context=None):
    #21-3-16 esta funcion para registrar entrada en almacen de producto devueltos
    ...
    search_condition =[('complete_name','ilike','stock')]
    whare_obj_dest = whare_obj.search (cr, uid, search_condition)
    if len(whare_obj_dest)==0:
        raise osv.except_osv(('Error!'),('No se ha definido un almacén para existencias. Revise la configuración de almacenes'))
    move_obj = self.pool.get('stock.move')
    #buscamos un tipo de movimiento de entrada en almacen
    picking_type_obj = self.pool.get('stock.picking.type')
    search_condition =[('code','like','incoming')]
    ...
    #_logger.error('##### AIKO ###### No se encuentra en stock.picking el orinen:%s'%name_pedido)
