...
class SaleOrder(models.Model):
    ...
    warrant_period = fields.Char(
        string='Warranty',
        help="Define warrant period for quotation lines",
        default=lambda self: self._default_warrant_period())

    @api.model
    def _default_warrant_period(self):
        company_pool = self.env['res.company']
        company_id = company_pool._company_default_get('sale.order')
        company = company_pool.browse(company_id)
        if company.default_sale_order_warrant_period:
            return company.default_sale_order_warrant_period
