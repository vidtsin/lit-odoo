...
class StockPicking(models.Model):
    ...
    amount_gross = fields.Float(
        string='Gross Amount', compute='_amount_all',
        digits=dp.get_precision('Sale Price'), help='The gross amount without tax.')
    amount_discount = fields.Float(
        string='Discount Amount', compute='_amount_all',
        digits=dp.get_precision('Sale Price'), help='The discounted amount before taxes.')
    amount_untaxed = fields.Float(
        string='Untaxed Amount', compute='_amount_all',
        digits=dp.get_precision('Sale Price'), help='The net amount without tax.')
        ...
        for picking in self:
            ...
            for line in picking.move_lines:
                if line.procurement_id and line.procurement_id.sale_line_id:
                    ...
                    discount = sale_line.price_unit * line.product_qty *(
                        (sale_line.discount or 0.0)/100.0)  
                    ...
                    val3 += cur.round(discount)
                    for tax in taxes['taxes']:
                        val2 += tax.get('amount', 0.0)
                    logger.error('Valor de val %s'%val)
                    logger.error('Valor de val1 %s'%val1)
                    logger.error('Valor de val2 %s'%val2)
                    logger.error('Valor de val3 %s'%val3)

            picking.amount_discount = val3
            picking.amount_untaxed = val1
            picking.amount_gross = val1 + val3
