...
class ResParnterBank(models.Model):
    _inherit = 'res.partner.bank'

    use_in_report = fields.Boolean(string='Use in reports', default=False)
...
class ResPartner(models.Model):
    ...
    @api.depends('bank_ids','bank_ids.use_in_report')
            ...
            if record.bank_ids:
                if len (record.bank_ids)==1:
                    ...
                    for bank in record.bank_ids:
                        if bank.use_in_report:
                            def_count = 1
                            record.default_account_bank_name = bank.bank_name
                    if def_count == 0:
                        raise Warning(
                _("No bank account in reports for this customer!"))
