...
class invoice_leader_export(osv.osv):
    ...
    _columns = {
        ...
        'tipo_iva': fields.char('Impuesto', readonly=True),
...
select 
            concat(inv.id::text,'00',tax.id::text)::int as id,
...
END as base,
        CASE WHEN inv.type = 'out_refund' THEN tax.amount *-1
ELSE tax.amount
END as iva,
	tax.name as tipo_iva,
...
(select res_id, value from ir_translation where name = 'res.country,name' and lang = 'es_ES') trans on trans.res_id = country.id left join
            ...
            account_period period on period.id = inv.period_id join
            account_invoice_tax tax on tax.invoice_id = inv.id
