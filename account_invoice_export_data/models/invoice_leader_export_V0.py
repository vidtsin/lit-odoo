...
class invoice_leader_export(osv.osv):
    ...
    _columns = {
        ...
        'imp_divisa': fields.float('Total en divisa', readonly=True),
...
END as total,
            CASE WHEN inv.type = 'out_refund' THEN inv.amount_total *-1
ELSE inv.amount_total
END as imp_divisa,
            divisa.name as divisa,
            payment.name as forma_pago, 
            inv.date_due as fecha_vto,
             CASE WHEN inv.type = 'out_refund' THEN inv.residual *-1
ELSE inv.residual
END as saldo,
...
END as cantidad,
            round(line.price_unit,3) as precio_ud,
        CASE WHEN inv.type = 'out_refund' THEN round(((line.quantity *line.price_unit) / cambio.rate)*-1,2)
ELSE round(((line.quantity *line.price_unit) / cambio.rate),2)
END as base_euros,
        CASE WHEN inv.type = 'out_refund' THEN round(line.price_subtotal *-1,2)
ELSE round(line.price_subtotal,2)
END as base_divisa,
            divisa.name as divisa,
            round(cambio.rate,4) as cambio,
            country.name as pais
        from account_invoice_line line left join
            ...
            res_partner partner on inv.partner_id = partner.id left join
            res_currency divisa on divisa.id = inv.currency_id left join
(select DISTINCT ON (currency_id) currency_id, id, rate from res_currency_rate
where not currency_id is null
order by currency_id, id DESC) cambio on cambio.currency_id = divisa.id left join
            res_country country on country.id = partner.country_id left join
            res_country_state state on state.id = partner.state_id left join
            payment_mode payment on payment.id = inv.payment_mode_id left join
            account_period period on period.id = inv.period_id
